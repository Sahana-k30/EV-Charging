...
router.get('/', protect, async (req, res) => {
    ...
    try {
        ...
        if (allStationsFromDB.length === 0 && sampleStations.length > 0) {
            ...
            return res.render('stations/index', { 
                stations: stationsToRender, 
                user: req.user
                // title and vehicles removed from here
            });
        }
        ...
        console.error('DEBUG: Error in simplified GET /stations route:', err); // Reverted log
        req.flash('error_msg', 'Error loading stations. Please try again.');
        res.redirect('/dashboard'); // Reverted error handling
    }
});
...
router.get('/:id', protect, async (req, res) => { 
    ...
    try {
        ...
        req.flash('error_msg', 'Error fetching station details');
